function [BH,mean_dist]=sc_compute1(GridPoints, BoundaryPoints ,mean_dist,nbins_theta,nbins_r,r_inner,r_outer,out_vec);
% [BH,mean_dist]=sc_compute1(Bsamp,Tsamp,mean_dist,nbins_theta,nbins_r,r_inner,r_outer,out_vec);
%
% modified version which computes hists on grid of points in image to
% contour points. (Tomer's version).
% compute (r,theta) histograms for points along boundary 
%
% Bsamp is 2 x nsamp (x and y coords.)
% Tsamp is 1 x nsamp (tangent theta) - REMOVED!!! NOT USED HERE!!!!!!
...
nGrid=size(GridPoints,2);
nBoundary = size(BoundaryPoints,2);
in_vec=out_vec==0;

% compute r,theta arrays
r_array=real(sqrt(dist2(GridPoints',BoundaryPoints'))); 
%r_array=real(sqrt(dist2(Bsamp',Bsamp'))); % real is needed to
                                          % prevent bug in Unix version
theta_array=atan2(GridPoints(2,:)'*ones(1,nBoundary)-ones(nGrid,1)*BoundaryPoints(2,:),...
                      GridPoints(1,:)'*ones(1,nBoundary)-ones(nGrid,1)*BoundaryPoints(1,:))';
...
r_array_q=zeros(nGrid,nBoundary);
for m=1:nbins_r
