...
for k=1:length(X)
    ...
    if strcmp(style,'patch')
      h(k).sdPtch=patchMaker(SD(k),[255,182,193]./255);
    end
    
    if strcmp(style,'patch') | strcmp(style,'sdline')
       
      h(k).semPtch=patchMaker(SEM(k),[255,182,193]./255);
      h(k).mu=plot([X(k)-jitter,X(k)+jitter],[mu(k),mu(k)],'-r',...
           'linewidth',2);
    end
    ...
    h(k).data=plot(thisX+J,...
                   thisY,'o','color',C,'markerfacecolor',...
                   C+(1-C)*0.65);
end
...
if strcmp(style,'line') | strcmp(style,'sdline')
  ...
  for k=1:length(X)    
    ...
    %Tomer's hack, add whiskers on SD lines:
    h(k).sdWhisker1 = plot([X(k)-(jitter*0.5),X(k)+(jitter*0.5)],[mu(k)+SD(k),mu(k)+SD(k)],'-','color',[0.2,0.2,1],'linewidth',2);
    h(k).sdWhisker2 = plot([X(k)-(jitter*0.5),X(k)+(jitter*0.5)],[mu(k)-SD(k),mu(k)-SD(k)],'-','color',[0.2,0.2,1],'linewidth',2);
  end
end
...
function ptch=patchMaker(thisInterval,color)
    l=mu(k)-thisInterval;
    u=mu(k)+thisInterval;
    ptch=patch([X(k)-jitter, X(k)+jitter, X(k)+jitter, X(k)-jitter],...
           [l,l,u,u], 0);
