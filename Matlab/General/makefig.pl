...
my ($right, $middle, $left) = (0, 0, 0);
my ($rightbar, $leftbar) = ("true", "true");
my $numbering;
my $startnum = 1;
GetOptions("left" => \$left, "right" => \$right, "middle" => \$middle, "from:s" => \$startnum, "nonumbers" => \$numbering);
...
# my @nts = qw(A C G T);
my @nts = qw(A R N D C Q E G H I L K M F P S T W Y V);
my (%p, @h);
...
my $dimensions;
{
    ...
    my $height = ($numbering eq "true") ? 0 : 40;
    $dimensions = join(" ", $left, $height, $right, 377);
}
