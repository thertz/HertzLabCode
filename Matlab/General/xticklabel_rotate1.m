function hText = xticklabel_rotate1(axisHandle,XTick,rot,varargin)
...
if (nargin < 3 || isempty(varargin{1})) & (~exist('XTick') | isempty(XTick)),
	xTickLabels = get(ax,'XTickLabel')  ; % use current XTickLabel
...
if (~exist('XTick') | isempty(XTick)),
    XTick = get(ax,'XTick')        ; % use current XTick 
end
...
% of the xlabel
hxLabel = get(ax,'XLabel');  % Handle to xlabel
...
set(get(ax,'xlabel'),'units','data')           ;
    labxorigpos_data = get(get(ax,'xlabel'),'position')  ;
set(get(ax,'ylabel'),'units','data')           ;
    labyorigpos_data = get(get(ax,'ylabel'),'position')  ;
set(get(ax,'title'),'units','data')           ;
    labtorigpos_data = get(get(ax,'title'),'position')  ;

set(ax,'units','pixel')                        ;
set(hText,'units','pixel')                      ;
set(get(ax,'xlabel'),'units','pixel')          ;
set(get(ax,'ylabel'),'units','pixel')          ;

origpos = get(ax,'position')                   ;
textsizes = get(hText,'extent');
if(iscell(textsizes))
  textsizes = cell2mat(textSizes);
end
longest =  max(textsizes(:,4))                  ;

laborigext = get(get(ax,'xlabel'),'extent')    ;
laborigpos = get(get(ax,'xlabel'),'position')  ;


labyorigext = get(get(ax,'ylabel'),'extent')   ;
labyorigpos = get(get(ax,'ylabel'),'position') ;
...
laborigpos = get(get(ax,'xlabel'),'position')  ;
set(get(ax,'xlabel'),'position',[laborigpos(1) laborigpos(2)-longest 0])   ;

% switch to data coord and fix it all
set(get(ax,'ylabel'),'units','data')                   ;
set(get(ax,'ylabel'),'position',labyorigpos_data)      ;
set(get(ax,'title'),'position',labtorigpos_data)       ;

set(get(ax,'xlabel'),'units','data')                   ;
    labxorigpos_data_new = get(get(ax,'xlabel'),'position')  ;
set(get(ax,'xlabel'),'position',[labxorigpos_data(1) labxorigpos_data_new(2)])   ;


% Reset all units to normalized to allow future resizing
set(get(ax,'xlabel'),'units','normalized')          ;
set(get(ax,'ylabel'),'units','normalized')          ;
set(get(ax,'title'),'units','normalized')          ;
