function [Energies]=get_eng_both_ways_specific_model(exp_binding_energies,B_Glo,B_Spe,peptides,input_matrix);
...
index_aa=reshape((1:dim)',num_aa,num_aa);
...
for j=1:len       
   ...
   if temp_length
      ...
      for kk=1:temp_length
         ...
         %bi((transpose(index_aa(ind1,ind2))-1)*dim12+(inter-1)*num_peptides+ref)=ones(1,num_peptides)*exp_binding_energies.h{j}(kk);
         aa_indxs(:,inter) =transpose(index_aa(ind1,ind2));
      end
   end
end
