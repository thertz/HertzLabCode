...
experimentDates = {'10_14_2014'}; %dates of experiments, each one is a directory where GPR files are saved.
expPrefix    = {'ImmHist_10_14_14_'}; % all slides and slideToSampleMapping file for each date have the exact same filePrefix
...
ECpathogens = {'K_2', 'K_3', 'K_4','VZV','H1_1','H1_2','H2re'};

ECinds = [];
for i=1:length(ECpathogens)
  currInd = strmatch(ECpathogens{i},{pathogenStruct.name});  
  if(i==1)
    EC_lysateInd = pathogenStruct(currInd).begInd;
  elseif(i==2)
    currInd = currInd(1);
  end
...
[figInd] = plotAllResponsesByGroup(mPeptResponses(:,1:67),groupNames,Flugroups,projectName,figInd,yLims,fontSize);% limits it to recon proteins

ReconInds= arrayData.antigenNames(1:67);
  
HIVinds = strmatch('HIV',arrayData.antigenNames);
Fluinds = strmatch('Flu',arrayData.antigenNames);
...
if (clusterPlotFlag)   
  ...
  %-------------------------------------------------------------------------------%
if (simplePlotFlag)
  [figInd] = plotAllResponsesByGroup(mPeptResponses(:,HSVinds),groupNames,expGroups,projectName,figInd,yLims,fontSize);%
