...
function [seqGPRstruct,controlGPRstruct] = processDENGUE_2014_GPRstruct_Multiplex(gprStruct,arrayNumber,numArrays)
...
controlInds = strmatch('D-C',pepNames); % all control peptides start with Control
dengueInds  = strmatch('D',pepNames); % all Dengue peptides start with D

%remove the D-C inds from the degueInds:
dengueInds = setdiff(dengueInds,controlInds);
...
controlNames = controlGPRstruct.Names;
for i=1:length(controlNames)
	% first 3 characters of the each control name are D-C, which we need to remove to get the number:
	controlNums(i) = str2num(controlNames{i}(4:end));
end
[Y cInds] = sort(controlNums);
