...
if(ZachFlag)
  ...
  %֜rootPath = [filesep,'Volumes',filesep,'Data',filesep,'HertzLab',filesep]; % main rootPath under which data is located 
  rootPath = [filesep,'Users',filesep,'thertz',filesep,'Dropbox',filesep,'HertzLab',filesep]; % main rootPath under which data is located 
  savePath = [rootPath,'ArrayData',filesep,'Influenza',filesep,projectName,filesep];
  loadPath = savePath;
end
%multiple dates can be entered, but must have a corresponding prefix
experimentDates = {'08_22_2014','08_25_2014','09_04_2014' '09_05_2014'}; %dates of experiments, each one is a directory where GPR files are saved.
expPrefix    = {'H7N9_08_22_14_','H7N9_08_25_14_','H7N9_09_04_14_', 'H7N9_09_05_14_'}; % all slides and slideToSampleMapping file for each date have the exact same filePrefix
...
adjuvantLabels    = {'Vac','PBS','MF59','AS03'}; % types of adjuvants used on both Obest and WT mice
...
if (clusterPlotFlag)  
  ...
  [figInd] = plotSummaryResponsesByClusters(mPeptResponses(postInds,:),clusterLabelsSha,titleStr,summaryMethod,figInd,yLims,fontSize);
  [figInd] = plotAllResponsesByClusters(mPeptResponses(postInds,:),clusterLabelsSha,titleStr,figInd,yLims,fontSize);
end
...
if (plotFlag)
  ...
  for i=1:length(expGroupStruct)
    ...
    [figInd] = plotSummaryResponsesByGroup(mPeptResponses,groupNames,expGroupStruct(i).groupNames,projectName,summaryMethod,figInd,yLimsSummary,fontSize);
  end
  ...
  for i=1:length(expGroupPrefixes)
    ...
    [figInd] = plotSummaryResponsesByGroup(mPeptResponses,groupNames,currGroupNames,projectName,summaryMethod,figInd,yLimsSummary,fontSize);
