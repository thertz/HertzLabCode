...
%
% for Dengue mouse samples from the Harris Lab - updated to work on new data generated on in-house printed slides Dec. 2014
...
experimentDates = {'12_01_2014','12_03_2014','12_08_2014','12_09_2014'}; %dates of experiments, each one is a directory where GPR files are saved.
expPrefix    = {'dengue_zm_12_01_14_','dengue_zm_12_03_14_','dengue_zm_12_08_14_','dengue_zm_12_09_14_'}; % all slides and slideToSampleMapping file for each date have the exact same filePrefix
...
expGroups = {'Mouse_PLO46', 'Mouse_Addavax','Mouse_Sigma_CPG', 'Mouse_Alum', 'Mouse_Ova', 'Mouse_Naive'}; % for new runs on in-house slides Dec. 2014
...
treatmentGroups = {'Mouse_Alum','Mouse_Addavax'}; % Names of the two groups that are to be compared statistically below (currently supports only pairwise comparisons)
...
% first clean data to include only mouse samples, as some runs included both human/mouse
mouseInds = strmatch('Mouse',groupNames);

slideNames     = slideNames(mouseInds);
ptids          = ptids(mouseInds);
groupNames     = groupNames(mouseInds);
mPeptResponses = mPeptResponses(mouseInds,:);

% now recompute indices of neg controls after removing all mouse/mAb data:
NegControlInds = strmatch('Mouse_Naive',groupNames); % 
...
if (plotFlag)
 ...
 %plotting and save figures
  ...
  [figInd] = plotSampleCorrmat(figInd,corrMatDen2(I,I),groupNames(I),[0 1],fontSize);
  ...
  for i=1:length(mousePepInds)
    [figInd] = plotGroupResponsesBoxPlots(mPeptResponses(allGroupInds,mousePepInds(i)),allGroupLabels,strrep(expGroups,'Mouse_',''),...
...
if (printFlag)
  figPath = [savePath,'figs',filesep,'Mouse',filesep];
  for j=1:figInd
