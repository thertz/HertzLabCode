...
ZachFlag = 0; % set to 1 if using a pc, set to 0 if using a mac
...
%-------------------------------------------------------------------------------------------------------------------------------------%
projectName = '< >'; %spelled the exact same as the folder it is in
...
experimentDates = {'< >'}; %dates of experiments, each one is a directory where GPR files are saved.
expPrefix    = {'< >'}; % all slides and slideToSampleMapping file for each date have the exact same filePrefix
...
processGPRfunctionHandle = @processFLU_HA_RapaGPRstruct_Multiplex; % < > needs to be updated for specific project
