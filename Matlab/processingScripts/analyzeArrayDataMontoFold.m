...
minFoldChangeThreshold = 0; % include all antigens - by setting this to zero...
...
for i=1:length(ptidStruct)
  ...
  if( isempty(preInd) || isempty(postInd) || isempty(ltInd) )
    ...
    ptidRespMatPost(i,:) = ptidStruct(i).postResponse;
    ptidRespMatLT(i,:)   = ptidStruct(i).ltResponse;

    ptidFoldMatPost(i,:) = ptidStruct(i).postResponse ./ mPeptResponses(preInd,:);
    ptidFoldMatLT(i,:)   = ptidStruct(i).ltResponse ./ mPeptResponses(preInd,:);
...
ptidFoldMatPost(remInds,:) = [];
ptidFoldMatLT(remInds,:)   = [];

ptidFoldMatPost(ptidFoldMatPost == Inf) = 0;
ptidFoldMatLT(ptidFoldMatLT == Inf) = 0;
...
% Cluster based on responses to the H3 - including HA and NA - note we are clustering fold change in post responses:    
...
[distMatVicPost,ZstructVicPost,clusterLabelsVicPost] = clusterSamplesByResponseVectors(ptidFoldMatPost(:,[Vic_HA_Inds ; Vic_NA_Inds]),numClusters,minFoldChangeThreshold);
...
[corrMatVicPost] = computeSampleCorrMatByResponseVectors(ptidFoldMatPost(:,[Vic_HA_Inds ; Vic_NA_Inds]),minFoldChangeThreshold);
...
totalMagnitudePost = sum(ptidRespMatPost - ptidRespMatPre,2);
H3N2magnitudePost  = sum(ptidRespMatPost(:,VicInds)-ptidRespMatPre(:,VicInds),2);
...
posFoldChange = 2;  
totalBreadthPost = sum(ptidFoldMatPost>posFoldChange,2);
H3N2breadthPost = sum(ptidFoldMatPost(:,VicInds) > posFoldChange,2);
...
if(plotFlag)
  ...
  [figInd] = plotSummaryResponsesByGroup(ptidRespMatPre(:,VicInds),groupNames([ptidStruct.preInd]),unique(groupNames([ptidStruct.preInd])),...
                                        [projectName,' Pre-vaccination'],summaryMethod,figInd,yLimsSummary,fontSize);
  [figInd] = plotSummaryResponsesByGroup(ptidFoldMatPost(:,VicInds),groupNames([ptidStruct.postInd]),unique(groupNames([ptidStruct.postInd])),...
                                        [projectName,' Post-vaccination'],summaryMethod,figInd,yLimsFold,fontSize);
  [figInd] = plotSummaryResponsesByGroup(ptidFoldMatLT(:,VicInds),groupNames([ptidStruct.ltInd]),unique(groupNames([ptidStruct.ltInd])),...
                                        [projectName,' LongTerm'],summaryMethod,figInd,yLimsFold,fontSize);
  ...
  [figInd] = plotAllResponsesByClusters(ptidRespMatPost(:,VicInds),clusterLabelsVicPre,'H3N2 Post - ordered by Pre',figInd,yLims,fontSize)
