...
experimentDates = {'04_16_2014'}; %dates of experiments, each one is a directory where GPR files are saved.
expPrefix    = {'rapa_zm_04_16_14_'}; % All slides and slideToSampleMapping file for each date have the exact same filePrefix
...
fontSize = 16;
expGroups = {'NC_Bc','NC_B6'};
addGroups = {'PBS','NC_Bc','NC_B6'};
yLims = [0 35000];
minThreshold             = 5000;
%handle to function that can process the GPR file. Each project will have a specific function.
%processGPRfunctionHandle = @processFLU_HA_RapaGPRstruct_Multiplex;  already present in processing script
%-------------------------------------------------------------------------------%
...
% plot all responses by group - limited only to the viet1203 inds
  [figInd] = plotAllResponsesByGroup(mPeptResponses(:,vietInds),groupNames, newGroups,'Adjuvants',figInd,yLims,fontSize); 
  
  % plot all responses by group after background subtraction
  NCinds = strmatch('NC',groupNames);
  mPeptResponsesBgSubtracted = mPeptResponses -repmat(max(mPeptResponses(NCinds,:)),length(ptids),1);

  % put back the NC responses unsubtracted!
  % mPeptResponsesBgSubtracted(NCinds,:) = mPeptResponses(NCinds,:);
  
  [figInd] = plotAllResponsesByGroup(mPeptResponsesBgSubtracted(:,vietInds),groupNames,addGroups,'Adjuvants',figInd,yLims,fontSize);
  ...
  if (printFlag)
    figPath = [savePath,experimentDates{i},filesep,'FIG',filesep];
    for j=3
