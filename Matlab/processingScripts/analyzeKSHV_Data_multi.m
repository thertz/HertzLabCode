...
fontSize                 = 16;
expGroups                = {'HHV8_pos','Low_risk','NC'};% groups listed in sample to slide mapping
...
EC_label    = 'null';   % name of antigen label of the EC control spots
...
for i=1:length(experimentDates)
  ...
  [arrayData] = normalizeEC_lysateBGresponses(arrayData, EC_label, method, EC_antigens);
  
  slideNames            = [slideNames arrayData.slideNames];
  ptids                 = [ptids arrayData.ptids];
  groupNames            = [groupNames arrayData.groupNames];
  mPeptResponses        = [mPeptResponses ;[arrayData(1).responseMatrix_EC_subtracted{1}]]; % NOTE USE OF SUBTRACTED ARRAY
...
if (simplePlotFlag)
  ...
  %[figInd] = plotAllResponsesByGroup(mPeptResponses,groupNames,expGroups,projectName,figInd,yLims,fontSize); %generic version
  ...
  EC_antigens_Full = [EC_antigens 'Null']; %add back Null for plotting
  
  for i= 1:length(EC_antigens_Full)
    
    % plot only highest titration - in this case labeled _4 - e.g K_1_4
    
    currInd = strmatch([EC_antigens_Full{i},'_4'],arrayData.antigenNames)

  % HACK HACK HACK FOR K5 AND K6
    if(strcmp(EC_antigens_Full{i},'K_5') || strcmp(EC_antigens_Full{i},'K_6'))
      currInd = strmatch([EC_antigens{i},'_14'],arrayData.antigenNames)
    end
